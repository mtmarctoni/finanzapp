...
export async function getRecurringRecords(
  user_id: string,
...
export async function generateFinanceEntries(
  user_id: string,
  date: string,
  year: number,
  month: number,
  ...
  try {
    ...
    const recordsToGenerate = activeRecordsResult.rows.filter(
      (record) =>
