...
export function useRecurringRecords(): UseRecurringRecordsResult {
  ...
  const deleteRecord = useCallback(
    async (id: string) => {
      if (!confirm('¿Seguro que deseas eliminar este registro recurrente?')) return false
  ...
  const generateRecords = useCallback(
    async (date: Date) => {
      if (!confirm('¿Generar ahora los registros recurrentes para la fecha seleccionada?')) return false
